...
Every transport enforces the shared deployment-mode and tenant-trust primitives. An absent selector maps to
the configured `[server.tenant] default_tenant` only in single-tenant mode; multi-tenant mode rejects it. Isolation is *structural* (the tenant is composed
