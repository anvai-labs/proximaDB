...
| Entities
| `/api/v1/collections/{id}/entities` (plus `/{entity_id}` and `/search`); gRPC v2 `ProximaEntityService`
| Transitional entity facade (upsert/search/get); the REST route is the sole residual v1 compatibility surface
| Beta / compatibility
