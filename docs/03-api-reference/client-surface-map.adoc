...
| pgwire | startup `database` (fallback tenant session variable) | A supplied database is the literal tenant assertion (`database=proximadb` means tenant `proximadb`). Only an omitted database selects the configured default in single-tenant mode; multi-tenant mode rejects an omission. See link:./postgres.adoc[Postgres Wire Reference].
|====
