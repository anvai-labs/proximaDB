...
| Records
| `/collections/{id}/records/batch`, `/collections/{id}/records/scan`, `/collections/{id}/records/{record_id}`
...
| Query
| `/query`, `/query/explain`
| UQL/Federated execution + plan explain; AQL execution is rejected until implemented (the explain request currently accepts the language label but has no AQL-specific lowering)
| Beta (AQL Planned)
...
| Entities
| Canonical `/collections/{id}/entities` (plus `/{entity_id}` and `/search`); gRPC v2 `ProximaEntityService`; a transitional REST v1 path family also includes entity listing
...
Every transport enforces the shared deployment-mode and tenant-trust primitives. When both an
explicit selector and an authenticated tenant binding are absent, single-tenant mode selects the
configured `[server.tenant] default_tenant`; multi-tenant mode rejects the request. Isolation is *structural* (the tenant is composed
...
| REST v2 | `X-Tenant-ID` header (optional) | An authenticated tenant binding takes precedence and a mismatch is rejected, except that an authenticated gateway principal may delegate an acting tenant under gateway-only trust. Without a binding, acceptance follows the configured header trust policy.
...
| pgwire | startup `database` (fallback tenant session variable) | A supplied database is literal (`database=proximadb` means tenant `proximadb`). Open trust accepts it; authenticated-only and gateway-only reject non-default assertions because pgwire trust auth has no credential binding. The reserved literal `default` remains accepted. Only an omitted database selects the configured default in single-tenant mode; multi-tenant mode rejects an omission. See link:./postgres.adoc[Postgres Wire Reference].
|====
...
* **v1 (`/api/v1/*`)** — removed except for the transitional entity path family at
  `/api/v1/collections/{id}/entities`, `/{entity_id}`, and `/search` (including `GET` listing on
  the collection entity path). All new clients use v2;
