...
[cols="1,3"]
|===
|Status|Proposed — **amended by ADR-094 (2026-09-21)**: the "Layer A ships 1:1 to Iceberg" claim is scoped per-modality (see the amendment in Decision)
|Date|2026-06-30
...
[IMPORTANT]
====
**Amendment (ADR-094, 2026-09-21) — "ships 1:1 to Iceberg" requires a Parquet-encoded
Layer A, and therefore holds per-modality, not universally.**

Premise 3 above is correct that Iceberg needs only three filesystem operations, so a
durable core *can* be an Iceberg data file. But the Iceberg spec also constrains data files
to **Avro, ORC or Parquet**. A `PBLK`/`PXH1`-encoded Layer A is none of those, so it cannot
literally be an Iceberg data file — it would need conversion, which is exactly the 1:1
property this clause claims.

ADR-094 resolves this by making the encoding follow modality:

* **Relational-only tables** — Layer A is **Parquet-encoded**, carrying PAX layout
  discipline (MVCC columns, tenant column, per-column zone maps, row-group geometry). The
  1:1 Iceberg claim then holds literally, and there is no second copy.
* **Vector / multimodal tables** — Layer A stays `PBLK`/`PXH1`, because the ~100×
  random-access advantage is decisive there (ADR-033) and the co-located vector + filter
  stripes have no faithful Parquet representation. For this arm, "ships to Iceberg" means
  *converted on export*, not 1:1.

The Layer A/B split itself, and the inviolable co-location of filter and vector stripes in
Layer A, are unchanged.
====
